...
class JalaliDateHelper extends Component
{
     ...
     * @return int|mixed
     */
    public static function remaining_days_in_month($timestamp = '') {
        $day = (int) JalaliDateHelper::jdate('d', $timestamp, '', 'Asia/Tehran', 'en');
        $month = (int) JalaliDateHelper::jdate('m', $timestamp, '', 'Asia/Tehran', 'en');

        return self::NUMBER_OF_DAYS[$month] - $day;
    }
     ...
     * @throws \Exception
     */
    public static function calculateDays($timestamp = false, $daysCount = 90) {
        if (!$timestamp) {
            $timestamp = strtotime('tomorrow');
        }

        $daysCountArray = array();

        for ($day = 1; $day <= $daysCount; $day++) {
            $dayOfWeek = (new \DateTime(date('Y-m-d', $timestamp)))->add(new \DateInterval("P" . $day . "D"))
                ->format('w');

            $daysCountArray[$dayOfWeek]++;
        }
